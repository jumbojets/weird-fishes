...
void matmul() {
  for (int b = 0; b < BS; b++) {
    for (int i = 0; i < M; i+=32) {     // Z tile row
      for (int j = 0; j < N; j+=64) {   // Z tile col. process two at once on seperate z-accumulators
        ...
        for (int k = 0; k < K; k+=32) { // down cols of At and B
          ...
          for (int rb = 0; rb < 32; rb+=8) {
            ...
            for (uint64_t r = 0; r < 8; r++) {
              AMX_LDY(At + b*K*M + (k+rb+r)*M + i, r, 0);
              uint64_t xr1 = (2*r)%8; // TODO: there should be cleaner way of doing this
              uint64_t xr2 = (2*r+1)%8;
              AMX_LDX(B + b*K*N + (k+rb+r)*N + j, xr1, 0);
              AMX_LDX(B + b*K*N + (k+rb+r)*N + j + 32, xr2, 0);
        ...
        for (uint64_t r = 0; r < 32; r++) {
          AMX_STZ(C + b*M*N + (i+r)*N + j, 2*r, 0);
          AMX_STZ(C + b*M*N + (i+r)*N + j + 32, 2*r+1, 0);
        }
    ...
    for (int b = 0; b < BS; b++) {
      for (int i = 0; i < M; i++) {
        for (int j = 0; j < N; j++) {
          _Float16 real = 0;
          for (int k = 0; k < K; k++)
            real += At[b*K*M + k*M + i] * B[b*K*N + k*N + j];
